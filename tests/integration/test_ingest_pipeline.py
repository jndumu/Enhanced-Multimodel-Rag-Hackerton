...
async def test_ingest_pipeline_produces_chunks_and_upserts():
    ...
    with patch("doc_intel_rag.api.routes.ingest.DocumentParser") as MockParser:
        ...
        with patch("doc_intel_rag.api.routes.ingest.enrich_chunks", new_callable=AsyncMock) as mock_enrich:
            mock_enrich.side_effect = lambda chunks, settings=None: chunks

            with patch("doc_intel_rag.api.routes.ingest.enrich_chunks_concepts", new_callable=AsyncMock) as mock_concepts:
